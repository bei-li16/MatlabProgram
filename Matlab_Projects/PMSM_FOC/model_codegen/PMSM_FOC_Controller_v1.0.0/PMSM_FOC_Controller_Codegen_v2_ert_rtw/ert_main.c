...
#include "PMSM_FOC_Controller_Codegen_v2.h" /* Model header file */
...
void rt_OneStep(void)
{
  ...
  /* Check for overrun */
  if (OverrunFlag) {
    rtmSetErrorStatus(PMSM_FOC_Controller_Codegen__M, "Overrun");
    return;
  }
...
int_T main(int_T argc, const char *argv[])
{
  ...
  fflush((NULL));
  while (rtmGetErrorStatus(PMSM_FOC_Controller_Codegen__M) == (NULL)) {
