...
本手册覆盖 \code{PMSM_FOC_DualPlant_v2.1.0} 模型包，说明组件化 FOC 控制器、Stateflow 电机运行状态管理、统一数据字典、类型化 Bus 接口、双 PMSM Variant 被控对象、调度边界、保护阈值、需求追溯以及自动测试结果。设计依据为项目 \path{doc/PMSM_FOC_Software_Architecture.tex} 中定义的分层控制和电机状态管理原则：数值控制算法由 Simulink 组件负责；Stateflow 负责模式转换和监督 PWM 请求；快速安全门负责最终 PWM 许可与过流锁存。

本版本控制器采用 2 个类型化 Bus 输入和 1 个 Bus 输出。输入类型为 \code{ControlCommandBus} 和 \code{MeasurementBus}；输出类型为 \code{ControlStatusBus}。

外部 \code{FaultResetRequest} 映射到 Stateflow 内部 \code{ResetAck}；\code{CalibrationBus} 作为数据字典中的等价标定结构，不增加顶层零散参数线。闭环 Harness 将状态、校准、对齐和故障测试信号统一收拢到 \code{Motor_Stateflow_Test_Logging} 子系统。两个电机对象及该日志子系统仅属于仿真环境，不进入控制器 ERT 目标代码。
...
\path{PMSM_FOC_Data.sldd} 是控制器设计数据的唯一来源。字典包含 31 个控制、保护、校准和数学参数，以及 39 个接口元素；参数目录逐项记录名称、目标类型、单位、范围、默认值、所有者、版本、数据类别和说明。参数集版本为 \code{2.1.0}，兼容性标识为 \code{PMSM_FOC_DUALPLANT_V21}。CRC 字段明确保留给后续标定导出流水线，不作为当前构建的虚假校验值。自动门禁确认控制器和 Harness 已绑定同一字典，迁移参数在两个模型工作区中的影子数量为 0；Harness 专用故障/偏置激励不迁入设计字典。
...
\midrule
\code{ControlCommandBus} & Start/Stop/EmergencyStop、Direction、Speed/Torque Reference、FaultResetRequest & 外部命令；本版本接入速度给定和复位，完整显式命令优先级由 ARC-003 实施 \\
\code{MeasurementBus} & Ia/Ib/Ic、Vdc、电角度、机械速度、Valid、Timestamp、Freshness & 采样与位置反馈契约；有效性/新鲜度的安全降级由后续测量任务补齐 \\
\code{ControlStatusBus} & DutyA/B/C、Id/Iq、Vd/Vq、PwmEnable、StateCode、FaultBits/Code、限幅和测量状态 & 最终控制输出与只读诊断；1 ms 状态通过保持边界同步到 100 $\mu$s 根输出 \\
...
由于 Duty 必须按 100 $\mu$s 输出，而状态码和慢故障每 1 ms 更新，\code{StatusState_To_100us} 与 \code{StatusFault_To_100us} 仅保持慢任务最近值，不增加 Stateflow 或慢保护的执行频率。本轮只冻结接口形状：\code{CurrentLimitActive} 暂映射快速过流锁存，\code{VoltageLimitActive} 暂为 false。完整限幅/饱和诊断由 ARC-003 和 CTL-001/002 实施，调用方不得把占位量当作已完成的降额控制。

\section{Stateflow 电机状态管理}
\sectionline

\code{Motor_Supervisor_1ms} 是运行模式和监督 PWM 请求的所有者，不直接拥有最终 PWM 许可。顶层 \code{SUPERVISED} 父状态包含 INIT、READY、CALIB、ALIGN 和 RUN，并以一条统一故障迁移进入顶层 FAULT。Chart 另输出 \code{CalibrationEnable/Reset}、\code{AlignmentEnable} 和 \code{ControllerReset}；数值平均、电压选择和最终快速门控仍由独立 Simulink 组件负责。FAULT 不会因物理故障自行消失而退出，必须同时满足外部 \code{FaultResetRequest}（映射为内部 \code{ResetAck}）、故障源消失以及启动命令撤销。
...
\code{Fast_Safety_Gate_100us} 监测 A/B 相过流，执行同周期最终 PWM 禁止并锁存快速故障；只有故障源消失且收到 \code{FaultResetRequest} 才能清除锁存。\code{Slow_Safety_Monitor_1ms} 监测过速和母线欠/过压，并把结果送入 Stateflow。Stateflow 根据慢保护或跨速率送入的快速故障转换到 FAULT；ALIGN/RUN 以外或最终许可为假的状态均使用 \code{PMSM_SafeDuty}。因此慢速模式管理不会增加快速过流的关断延迟，物理故障源恢复也不会触发自动重启。
...
\midrule
机械转速 & \code{PMSM_Protection_}\newline\code{MaxSpeed_Rpm}（默认 3000 rpm） & 1 ms 慢保护 & 请求进入 FAULT，输出安全占空比 \\
A/B 相电流 & \code{PMSM_Protection_}\newline\code{MaxCurrent_A}（默认 12 A） & $100\,\mu\mathrm{s}$ 快速安全门 & 当前快周期关断并锁存；输出安全占空比 \\
直流母线 & \code{PMSM_Protection_}\newline\code{MinDcBus_V / MaxDcBus_V}（默认 10/60 V） & 1 ms 慢保护 & 请求进入 FAULT，输出安全占空比 \\
\bottomrule
\end{tabularx}
\end{table}
...
状态/速率结构 & 6 状态、13 个控制组件、2 Bus 输入/1 Bus 输出；Chart/慢保护为 1 ms、快保护为 $100\,\mu\mathrm{s}$ & \pass \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{itemize}[nosep,leftmargin=2em]
  ...
  \item 数据字典含 31 个参数和 39 个接口元素，四类 Bus 逐元素检查通过，迁移参数工作区影子数为 0；
  \item 速度、Stateflow 和慢保护任务均为 0.001 s；电流与快速安全门任务均为 0.0001 s；
  \item 显式任务边界、双速率调度器和快/慢安全逻辑均进入生成代码；
  \item 控制器与闭环模型悬空线数均为 0；
  \item 生成代码包含三个 Bus 头文件、\code{initialize/step}、外部输入/输出和 Stateflow 逻辑，共有 1 次 \code{sinf} 与 1 次 \code{cosf} 调用，不包含 S-Function 文本，综合结论为 \pass。
\end{itemize}
...
\midrule
已满足 & 9 & M0 数据字典/Bus、M1 PI、M2 可切换对象、M3 FOC/双速率、M6 状态机/故障、M9 ERT 代码生成 \\
部分满足 & 6 & M1 对象级阶跃、M2 dq 对象验收、M4 测量有效性、M5 编码器/工况、M9 SIL/PIL/实时集成 \\
...
主要偏离包括：显式 Start/Stop/EmergencyStop 字段虽已进入 \code{ControlCommandBus}，但行为接入归 ARC-003，当前启动仍由速度阈值推导；故障位/码已进入状态 Bus，但稳定编号和历史归 ARC-004；\code{MeasurementBus.Valid/FreshnessTicks} 尚未驱动降级；CRC 是受控占位符而非已实现校验。上述缺口在矩阵中均以风险和后续 TODO 明确记录。

\subsection{BASE-001 可复现基线}

2026 年 8 月 31 日使用 \code{verify_pmsm_foc_reproducibility_v21} 连续执行两次 clean build。每轮均从 \path{PMSM_FOC_Native_v2.0.0} 源模型覆盖 v2.1.0 模型，然后确定性应用组件、Stateflow、双速率任务、安全逻辑、数据字典和 Bus 接口重构，运行全量 MIL 测试并复核 ERT 代码。重复构建关闭非功能性的架构截图导出以限制 MATLAB 峰值内存；结果曲线、模型/代码、测试和报告仍完整刷新。
...
本机双轮基线结论为 \pass。官方 MCB PMSM HDL 模块会报告约 $10^{-8}$ 量级的 single 参数量化提示，该提示不改变校验结果且不属于门禁失败。构建报告如实记录本任务修改尚未提交，因而 Git 工作区非 clean；正式发布前仍须在已提交且 Git clean 的第二台同配置开发机复跑，并确认两个 checksum、接口和全部门禁一致。
...
脚本会创建/更新数据字典，重建组件和 Stateflow 集成，应用 Bus 根接口，依次仿真两个 Variant，执行校准、对齐、1 ms 慢保护、$100\,\mu\mathrm{s}$ 快速过流、五状态故障锁存和确认复位测试，验证连线、任务周期、参数影子和四类接口契约，生成控制器 ERT C 代码并更新 \path{verification_report.txt}。Stateflow/Bus 集成脚本可重复运行：每次先恢复确定性的标量内部实现，再建立同一结构，因此不会积累红色虚线或重复连线。Harness 的 18 路测试信号集中在一个日志子系统内，不进入 ERT 控制器。
...
v2.1.0 已明确职责边界：1 ms Stateflow 管理模式、监督 PWM 请求和确认复位；1 ms 慢保护负责过速及母线电压；$100\,\mu\mathrm{s}$ 快速安全门拥有最终 PWM 许可并锁存过流；独立 Simulink 组件执行数据字典配置的偏置校准与 d 轴对齐；1 ms 速度环生成转矩电流请求；$100\,\mu\mathrm{s}$ 电流环完成 FOC 与 SVPWM。31 个设计参数、39 个接口元素、\code{2BI-1BO} 根接口和 M0--M10 的 22 条追溯需求已纳入手册。双被控对象、数值校准、实际对齐电压、零延迟快速过流关断、任意运行状态故障锁存、确认复位、结构、Bus C 类型和 ERT 代码生成均通过自动验收。显式命令行为、详细故障字典、两电阻采样、观察器和硬件部署仍按追溯矩阵延期，不属于当前完成范围。
